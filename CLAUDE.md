...
> 🔻 **续接/压缩后先读**：`docs/context/RESUME.md`（当前进度、下一步、工作节奏、地雷、真相源地图）。
...
> **两套规则的关系（防混淆——执行判断的硬约束）**：本节 7 条是**高层红线（原则层）**，贯穿全 SDLC，定"什么不可逾越"；Git 分支/合并的**具体执行护栏**另见 `docs/requirements/铁锤小队-Harness工程构思-v4.md` §9「驭手 8 军规」（执行层），定"在 Git 上怎么落地"。两者**独立编号**——4/5/6 号在两套含义完全不同（红线4=角色不混同 vs 军规4=短命快合；红线5=不弱化测试 vs 军规5=Feature Flags；红线6=阻塞升级 vs 军规6=规范命名+提交），**引用务必带"红线 N"/"军规 N"前缀，严禁裸用数字**（裸用会张冠李戴）。两层互补、不冲突；万一抵触，**以红线（原则）为准**并升级人类调整军规。
...
7. **人类门禁不可绕过**：产品范围、架构锁定、规约变更、安全敏感改动、生产部署必须人类签字。（此为原则层；合并/审查的 Git 执行细则——只提议、绝不直接动 main、永远看 diff、AI 代码加严审查——落在 §9 **军规 1/2/7**。）
