...
> **与 CLAUDE.md「核心禁止事项」的分层关系**：本节 8 军规是 **Git 执行层**护栏（"怎么在 Git 上落地"）；CLAUDE.md 7 红线是**高层原则层**（"什么不可逾越"）。两者**独立编号**，4/5/6 号含义不同（军规4=短命快合/红线4=角色不混同；军规5=Feature Flags/红线5=不弱化测试；军规6=规范命名+提交/红线6=阻塞升级），引用须带"军规 N"/"红线 N"前缀，勿裸用数字。军规落实红线在 Git 上的执行；抵触时以红线为准。
