...
> 本文件只记"活状态 + 隐性知识"；机制/规约细节一律去真相源,不在此重复。最后更新 2026-06-25(进度:M0–M5+M4+封板;M6 主体完成;M7 启动,M7-a 工具序列 + M7-b 人工干预率 + M7-c 语义相似度 sensor 完成,drift 三型补齐两型)。
...
>   - ✅ **M7-b 人工干预率(HIR) sensor 完成**(change `pipeline-drift-hir-sensor`,扩 `drift-monitor`;复盘 `M7-hir-retro.md`)。**HIR=KB「终极 drift 指标」**(~200 交互后 +216%)。`metrics/hir-sensor.ts` 纯 `hir`(干预数/总数,干预=`blocked-escalated` 红线6 升级;0 run→null)/`hirSeries`(按 ts 切 tumbling 满窗 HIR 序列,给 HIR 加时间维)/`risingAlert`(连续 k 窗 ≥θ 告警,**M7-a `driftAlert` 跌破 τ 的上升镜像**)+ 薄 `readHirRuns`(保 ts+按 jobId 去重;**既有 `readRunLedger` 投影丢 ts 不能复用**,同 M7-a readDriftEvents)+ `computeHir`(不足一窗→insufficient-data)。**核心:把无时间维的既有 `escalationRate` 升级为可测 drift 趋势的 sensor(同一公式+时间轴)**。θ=0.5/window=5 保守占位待标定(**KB 未给 HIR 绝对阈值**——这是不臆造的依据)、k=3 KB 默认。metrics gate 全绿(88,71→88 +17,既有零影响);真实空 ledger→`{insufficient-data,rate:null,alert:false}` 诚实不臆造。**口径窄(从窄到宽)**:仅 run 级 `blocked-escalated`,`held`(集成级,events.jsonl 另源)/`failed` 待拓宽;未接看板。下一候选 M7-c 语义相似度(需 embedding)/搭长程任务测试(解锁所有真信号)。
>   - ✅ **M7-c 语义相似度 sensor 完成(纯核+接口)**(change `pipeline-drift-semantic-sensor`,扩 `drift-monitor`;复盘 `M7-semantic-retro.md`)。KB 接地(wiki-query 读 `[[agent-drift]]`):**Semantic Drift=输出渐进偏离原意,检测法=embedding cosine similarity**,属 ASI Response Consistency(0.30)。`metrics/semantic-sensor.ts` 纯 `cosine`(零向量→0 不臆造;维度不等→抛,契约违例不静默吞)/`semanticConsistencySeries`(相对首基线)+ 注入接口 `EmbedFn=(text)=>number[]`(**不绑定模型**,离线纪律)+ 数据结构 `SemanticResponse{ts,traceId,text}` + 组装 `computeSemanticDrift`(复用 M7-a `driftAlert` 跌破 τ,**与工具序列同方向**——一个连续-k 框架已覆盖 drift 三型里两型)。**drift 三型补齐 Semantic**(M7-a/b=Behavioral)。**双重待埋点(外部条件最多的一刀)**:① 响应文本采集(需改 invoke stream-json 落 assistant 文本,碰核心路径,**用户裁定留后续独立切片**)② embedding provider 接入(离线选型待定)——embed 缺省/响应<2 → insufficient-data 不臆造。τ=0.75/k=3 沿用 ASI 框架默认待标定。100 测试(88→100,+12)既有零影响;合成 embedding 验证渐进偏离告警/稳定不告警。Confidence Calibration(JS 散度)第二条腿从窄到宽留后续。下一候选 M7-d 共识/协调(需多 agent)/搭长程任务测试+响应文本采集(解锁真信号)。
> （以下 M6 切片明细留作历史)
