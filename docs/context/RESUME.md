...
- 待办:**M4+ 续切片**(从窄到宽):② 追溯链自动织链(changeId→spec→tests→commit 自动采集,取代手维护 traces.json);③ Defect Escape 自动喂;持久化指标存储(events.jsonl ephemeral 的固有限制);metrics 包级 stryker 变异门。外部通知渠道;report 历史归档;comprehension debt 待合阈值告警;M6+。
