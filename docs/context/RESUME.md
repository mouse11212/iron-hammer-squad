...
- 待办:**M4+ 续切片**(从窄到宽):① Verification Tax 计算(据 events durationMs 聚合实现 vs 验证,collect.ts 去 `verificationMs=null`,schema 已留钩子);② 追溯链自动织链(changeId→spec→tests→commit 自动采集,取代手维护 traces.json);③ Defect Escape 自动喂。外部通知渠道;report 历史归档;comprehension debt 待合阈值告警;M6+。
