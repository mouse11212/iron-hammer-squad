...
- 待办:**M4+ 续切片**(从窄到宽):inner-loop 升级率/成本持久(需 escalated run → ledger 类机制,trailer 持久不了无提交的 run);metrics 包级 stryker 变异门。外部通知渠道;report 历史归档;comprehension debt 待合阈值告警;M6+。
