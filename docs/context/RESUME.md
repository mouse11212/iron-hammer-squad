...
- 待办:**M4+ 续切片**(从窄到宽):driver 自动 emit `Defect-Caught:` trailer(把 caught 也持久化,根治口径不对称);持久化指标存储(events.jsonl/runs ephemeral 的固有限制);metrics 包级 stryker 变异门。外部通知渠道;report 历史归档;comprehension debt 待合阈值告警;M6+。
