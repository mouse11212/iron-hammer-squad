...
## 续切片⑦ 已完成:report 历史归档（change `pipeline-report-history`）

`bin-report` 每次覆盖 dashboard.md → 只有"此刻",看不到趋势。本切片把每次 opt-in 归档的指标快照留时间序列,看板渲趋势——服务 ③ Compound 层"小队是否越用越强"的判断。

- **opt-in 触发(用户确认)**:`npm run report:archive` 才追加;普通 `report` 只重生看板+渲趋势(读不写)。避免随手 regen 凑重复行、dev 期跳测污染——归档是刻意里程碑采样。
- **slim 投影**:`historySnapshot` 只存四 KPI + 解决计数,丢 traces/taxByTrace/innerLoop 大字段(趋势不需,避免账本膨胀);null KPI 保留。
- **向后兼容的渲染扩展**:`renderBoard(snap, history?)` 加可选参数——既有无 history 调用(及 30+ board 测试)零行为变化,只 bin-report 传入。N=10 截显示,history.jsonl 留全量。
- **真实验证(非破坏)**:临时 repo archive 两次 → history 2 行 → 看板趋势表显示两行;空 history → 无趋势区。metrics gate 58 全绿。
- **与⑥ 的正交**:⑥ runs-ledger 存 per-run 原子事实("发生过哪些 run");⑦ history 存 per-report 聚合快照("每次结算大盘啥样")。同构(append-only/持久不可复现)但互不重复。
- **可观测闭环的两个时间轴**:切片①–⑥ 解决"单点可度量 + 持久";⑦ 加上"长期趋势"。外部通知(实时推送/出事喊人)是另一端,留 daemon 真无人值守长跑时做(涉对外发布安全裁决)。

## 下一候选（M4+ 零头 / M5+）

8. metrics 包级 stryker 变异门(E4 未配,现靠穷尽精确断言兜底);外部通知渠道(daemon 长跑时);daemon 自动归档采样;comprehension debt 待合阈值告警;M6+。
