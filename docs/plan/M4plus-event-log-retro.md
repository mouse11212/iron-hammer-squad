...
4. **driver 自动 emit `Defect-Caught:` trailer**(把 caught 持久化,根治 caught/escaped 口径不对称);持久化指标存储(events.jsonl/traces/runs 产物均 ephemeral 的固有限制);metrics 包级 stryker 变异门。
