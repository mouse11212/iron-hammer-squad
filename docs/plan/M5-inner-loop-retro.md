...
> 而非依赖静态 stryker.conf —— 否则新能力的测试强度无人把关。
> **✅ 已修复(2026-06-22)**:`gates.ts` 新增纯解析 `mutateTargetsFromStatus`(git status --porcelain → src/*.ts,
> 排除测试/非源/已删除/去引号);`mutation()` 据此用 stryker `--mutate <files>` 覆盖静态配置;无源改动则跳过。
> 单测 26(parser 9 + 动态行为);driver 变异门 92.64%(gates.ts 自身 90.70%);stryker `--mutate` 单文件机制实测确认。
> 顺带根因重构:删除 red/mutation step 的死字段 `label`(只 green 步用)。
> **✅ 真 e2e 确认 + 子目录 bug 修复(2026-06-22)**:跑 clampPercent US 真 e2e,gates.jsonl 暴露动态门跳过了新文件——
> 根因:`git status --porcelain` 返回**仓库根相对**路径(`iron-hammer-output/fincards/src/...`),parser 过滤 `src/` 前缀不匹配 → 空。
> 修:`mutation()` 先取 `git rev-parse --show-prefix`,parser 剥 prefix 得工程相对(顺带排除工程外改动)。
> 实测确认:修复后 gate 实跑 `npm run mutation -- --mutate src/clampPercent.ts`,ok。教训:**单测合成路径(`src/foo.ts`)永远测不出子目录坑,必须真集成验证**。
> 顺带加 gate 命令 trace(`gates.jsonl`):阶段间确定性 gate(lint/tsc/test/git/stryker)此前 TS 层执行未落盘,补此观测盲点。
> 回修闭环再次实证:clampPercent 首版变异门 84.62%(评审判 2 个为真等价变异)→ dev-1 resume 回修加 Stryker disable 显式豁免 → done/fixRounds=1/$3.48。
