...
铁锤小队流水线的**消息/事件/任务队列组件** = **嵌入式 SQLite 队列(事务原子认领 + WAL 崩溃恢复)封装为 stdio MCP server**。

> **落地修正(2026-06-17,BOSS 签字)**:实现库由原首选 `better-sqlite3` 改为 **Node 内置 `node:sqlite`**。
> 原因:落地实测环境 Node v24.15.0 已内置稳定 `node:sqlite`(需 ≥22.5)。对北极星目标「**可分发 CC 插件**」,内置模块严格优于原生 npm 依赖——省去 node-gyp 编译、跨平台预编译二进制、离线包体积负担;两者同样满足 D9 硬要求(事务原子认领 + WAL + `busy_timeout`)。
> 这是「抽取≠冻结 / 求真纪律」的一次应用:决策记录为真相源但非教条,落地实测推翻细节时就地修正并标注原因。`better-sqlite3` 保留为环境无内置 SQLite 时的回退选项。
